...
function monthsBetween(a, b) {
  // المادة 35: كسر الشهر الأخير يُعدّ كشهر كامل
...
// targetCat: الفئة المستهدفة لآخر 5 سنوات (اختياري — إذا لم يُمرَّر يُستخدم 15)
function buildCatPlan(sa, ta, sc, targetCat) {
  sa = Math.min(Math.max(sa, 0), 100);
  ta = Math.min(Math.max(ta, sa), 100);
  if (ta - sa > 60) ta = sa + 60;

  // الفئة المستهدفة: إذا حُدِّدت يجب ألا تقل عن sc
  const TC = (targetCat && targetCat >= 1 && targetCat <= 15)
    ? Math.max(targetCat, sc)   // لا يمكن أن تكون المستهدفة أقل من البداية
    : 15;
  ...
  const yearsBeforeLast5 = last5Start;
  // أعلى فئة يمكن الوصول إليها بالترقي التدريجي قبل آخر 5 سنوات
  const maxReachableAt5 = Math.min(TC, sc + yearsBeforeLast5);

  if (sc >= TC) {
    // ── الحالة 1: الفئة الحالية تساوي أو تتجاوز المستهدفة ─────────────────
    rows.forEach(r => r.cat = TC);

  } else if (yearsBeforeLast5 === 0) {
    // ── الحالة 2: المدة الكلية ≤ 5 سنوات ──────────────────────────────────
    // ترقّ تدريجي طوال المدة نحو TC
    let cat = sc;
  ...
  } else if (yearsBeforeLast5 >= (TC - sc)) {
    // ── الحالة 3: وقت كافٍ للوصول للمستهدفة TC ───────────────────────────
    let rem = yearsBeforeLast5 - (TC - sc);
    for (let i = 0; i < total; i++) {
...
/* ─────────────────────────────────────
   PENSION
───────────────────────────────────── */
// FIXED_CAT_FOR_AVG: لم يعد مطلوباً لأن buildCatPlan يُدير الفئة المستهدفة في آخر 5 سنوات مباشرة
// نبقيه null دائماً للتوافق مع الكود الآخر
let FIXED_CAT_FOR_AVG = null;
// TARGET_CAT_FOR_LAST5: الفئة المستهدفة لآخر 5 سنوات (null = 15 الافتراضي)
...
function calcPension(avg, mo) {
  ...
  // المادة 36: لا يقل عن الحد الأدنى للأجر (350,000) ولا يزيد عن 80% من متوسط الأجر
  // الحد الأدنى له الأولوية (مطلق) → نطبّق الحد الأقصى أولاً ثم الحد الأدنى
  p = Math.min(p, avg * 0.8);   // لا يزيد عن 80% من متوسط الأجر
  p = Math.max(p, 350000);       // لا يقل عن الحد الأدنى (350,000) — أولوية مطلقة
  return p;
}
...
function mkResult(svc, birth, sa, ta, buy, useUC, uc, joinDate) {
  ...
  // TARGET_CAT_FOR_LAST5: الفئة المستهدفة لآخر 5 سنوات (null = 15 افتراضياً)
  ...
  const css = `
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    ...
    .page-footer{
      ...
      font-size:8.5px;color:#7a6e5f;text-align:center;
    }
    /* ── تحسين عام للـ PDF ── */
    .plan-page { padding:14px 18px 10px; max-width:780px; margin:0 auto; }
    .plan-hdr-bar { padding:9px 14px; margin-bottom:10px; }
    .plan-hdr-bar h2 { font-size:15px; }
    .emp-bar { margin-bottom:8px; }
    .emp-cell { padding:6px 9px; }
    .emp-lbl { font-size:8.5px; }
    .emp-val { font-size:10.5px; }
    .stats-row { gap:4px; margin-bottom:8px; }
    .stat-box { padding:6px 7px; min-width:70px; }
    .stat-box .sl { font-size:7.5px; }
    .stat-box .sv { font-size:11px; }
    .details-grid { padding:7px 11px; margin-bottom:8px; }
    .detail-row { padding:3.5px 0; font-size:10px; }
    .rpill { padding:5px 10px; font-size:10px; margin-bottom:8px; }
    .tbl-title { font-size:9.5px; margin-bottom:4px; }
  ...
  <div class="cover">
    ...
    <div style="margin-top:20px;text-align:center;">
      <img src="data:image/jpeg;base64,${QR_B64}" 
           style="width:100px;height:100px;border:3px solid rgba(184,148,63,.5);border-radius:10px;padding:4px;background:rgba(255,255,255,.1);" 
           alt="QR Code"/>
      <p style="color:#d4af6a;font-size:11px;margin-top:8px;font-weight:600;">امسح الكود للتواصل | linktr.ee/Daman2026</p>
    </div>
  </div>`;
    ...
    <div class="plan-page">
      ...
      <div class="details-grid">
        ...
        <div class="detail-row">
          ...
          <span class="dv dv-g">
            ${N(plan.pension)} د.ع
            <span style="font-size:8px;color:#2d6a4f;font-weight:400;display:block;margin-top:2px">
              ${N(plan.avg)} × 2.5% × ${plan.totalServiceMonths} شهر ÷ 12
              ${(()=>{ const raw=plan.avg*0.025*plan.totalServiceMonths/12; const afterCap=Math.min(raw,plan.avg*0.8); return afterCap<350000?' — طُبِّق الحد الأدنى 350,000 د.ع (م.36)':(raw>plan.avg*0.8?' — طُبِّق الحد الأقصى 80% (م.36)':''); })()}
            </span>
          </span>
        </div>
        ${hasBuy?`
      ...
      <div class="page-footer">نظام جداول التقاعد الاختياري — ${today}
        <br/><br/>
        <a href="${QR_LINK}" style="display:inline-block;text-decoration:none;">
          <img src="data:image/jpeg;base64,${QR_B64}" 
               style="width:65px;height:65px;border:2px solid #b8943f;border-radius:7px;padding:2px;background:#fff;vertical-align:middle;" 
               alt="QR Code"/>
        </a>
        <span style="display:block;color:#b8943f;font-size:8px;margin-top:4px;font-weight:700;">امسح الكود للتواصل — linktr.ee/Daman2026</span>
      </div>
    </div>`;
  });
...
function buildSummary(nm, g, age, jat, ts, hasJoin) {
  ...
  // عمر الانتساب المحسوب من الخدمة السابقة (بدون تاريخ انتساب)
  if (!hasJoin && USER && USER.saDisplay) {
    items.push({ l: 'عمر بدء الخدمة (تقريبي)', v: USER.saDisplay });
  }
        ...
        <div class="dr">
          ...
          <span class="dv g">
            ...
            <span style="font-size:.7rem;color:var(--green);font-weight:400;display:block;margin-top:2px;opacity:.85">
              ${N(plan.avg)} × 2.5% × ${plan.totalServiceMonths} شهر ÷ 12
              ${(()=>{ const raw = plan.avg*0.025*plan.totalServiceMonths/12; const afterCap=Math.min(raw,plan.avg*0.8); return afterCap < 350000 ? ' — <span style="color:var(--amber)">طُبِّق الحد الأدنى 350,000 د.ع (م.36)</span>' : (raw > plan.avg*0.8 ? ' — <span style="color:var(--amber)">طُبِّق الحد الأقصى 80% (م.36)</span>' : ''); })()}
            </span>
          </span>
        </div>
        ${hasBuy ? `
...
/* تحويل التركيز (focus) وتمرير الصفحة للحقل الأول الذي يحتوي على خطأ */
function focusFirstError() {
  const first = document.querySelector('.ctrl.field-error:not(:disabled)');
  if (!first) return;
  const field = first.closest('.field, .date-slot, .sub-group');
  const target = field || first;
  const headerH = document.querySelector('header') ? document.querySelector('header').offsetHeight : 0;
  const y = target.getBoundingClientRect().top + window.pageYOffset - headerH - 24;
...
function generate() {
  try {
    ...
    if (hasError) {
      showAlert('❌ يرجى إدخال تاريخ الميلاد والجنس', 'e');
      focusFirstError();
      return;
    }
    ...
    if (isNaN(birth.getTime()) || birth > new Date()) {
      ...
      showAlert('❌ تاريخ الميلاد غير صحيح', 'e');
      focusFirstError();
      return;
    }
    ...
    // الفئة المحددة لآخر 5 سنوات (اختيارية)
    ...
    if (fixedCatYes && !fixedCatVal) {
      showAlert('⚠️ يرجى اختيار الفئة المستهدفة', 'w'); return;
    }

    // التحقق: الفئة المستهدفة يجب أن تكون ≥ الفئة الحالية
    if (fixedCatYes && fixedCatVal && useUC) {
      if (+fixedCatVal < uc) {
        showAlert(`❌ الفئة المستهدفة (${fixedCatVal}) لا يمكن أن تكون أقل من الفئة الحالية (${uc}). يرجى اختيار فئة أعلى أو مساوية.`, 'e');
        return;
      }
    }

    TARGET_CAT_FOR_LAST5 = (fixedCatYes && fixedCatVal) ? +fixedCatVal : null;
    FIXED_CAT_FOR_AVG = null; // دائماً null — الحساب يتم عبر buildCatPlan
    ...
    // حساب عمر الانتساب:
    // - إذا وجد تاريخ انتساب: نستخدمه مباشرة
    // - إذا وجدت خدمة سابقة بدون تاريخ انتساب: نحسب عمر الانتساب = العمر الحالي - سنوات الخدمة السابقة
    // - إذا لا شيء: نبدأ من العمر الحالي
    let sa, sm;
    if (ja) {
      sa = ja.years;
      sm = ja.months;
    } else if (prior > 0) {
      // عمر الانتساب التقريبي = العمر الحالي بالأشهر - الخدمة السابقة بالأشهر
      const ageNowMonths = ageNow.years * 12 + ageNow.months;
      const saMonths = Math.max(0, ageNowMonths - prior);
      sa = Math.floor(saMonths / 12);
      sm = saMonths % 12;
    } else {
      sa = ageNow.years;
      sm = ageNow.months;
    ...
    USER = { gender: g, nm, ay: ageNow.years, am: ageNow.months, ad: ageNow.days,
             ...
             // عمر الانتساب المحسوب (للعرض فقط)
...
// عدد أيام الشهر بحسب النظام العالمي
function daysInMonth(year, month) {
  return new Date(year, month, 0).getDate(); // month هنا 1-12
}

// عند اختيار السنة — تفعيل الشهر
function onYearChange(prefix) {
  ...
  // أعد ضبط اليوم إذا تغيّرت السنة
  dayEl.disabled = true; dayEl.value = '';
  dayEl.innerHTML = '<option value="">اليوم</option>';
}
window.onYearChange = onYearChange;

// عند اختيار الشهر — تفعيل اليوم وتحديث عدد أيامه
function onMonthChange(prefix) {
  ...
  // إذا كان اليوم المختار أكبر من عدد أيام الشهر الجديد — أعده
...
function reset() {
  ...
  // إعادة تهيئة تواريخ الميلاد
  document.getElementById('birthYear').value   = '';
  document.getElementById('birthMonth').value  = ''; document.getElementById('birthMonth').disabled = true;
  document.getElementById('birthDay').value    = ''; document.getElementById('birthDay').disabled   = true;
  document.getElementById('birthDay').innerHTML= '<option value="">اليوم</option>';
  // إعادة تهيئة تاريخ الانتساب
  document.getElementById('joinYear').value    = '';
  document.getElementById('joinMonth').value   = ''; document.getElementById('joinMonth').disabled  = true;
  document.getElementById('joinDay').value     = ''; document.getElementById('joinDay').disabled    = true;
  document.getElementById('joinDay').innerHTML = '<option value="">اليوم</option>';
...
/* ─────────────────────────────────────
   ...
   — الرابط يفتح صفحة viewer.html منفصلة
   — لا يُحوَّل المستخدم إلى الحاسبة نهائياً
───────────────────────────────────── */
function buildShareLink(chosenPlans) {
  ...
  const payload = {
    ...
    u: [
      ...
      USER.ay, USER.am, USER.ts,
      USER.py, USER.pm, USER.sa
    ],
    p: chosenPlans.map(p => [
  ...
  // بناء رابط viewer.html منفصل — لا علاقة له بالحاسبة
  // نستخرج المسار الأساسي من الرابط الحالي مع ضمان صحة الرابط
  const _loc    = window.location;
  const _base   = _loc.protocol + '//' + _loc.host +
                  _loc.pathname.replace(/\/[^/]*$/, '') + '/viewer.html';
  const baseUrl = _base;
  ...
  // توليد QR code لرابط المشاركة وعرضه في النافذة المنبثقة
  const qrWrap = document.getElementById('shareQrWrap');
  const qrImg  = document.getElementById('shareQrImg');
  if (qrWrap && qrImg) {
    // استخدم Google Charts API لتوليد QR code للرابط المشارَك
    const qrSize  = 120;
    const qrSrc   = 'https://chart.googleapis.com/chart?cht=qr&chs=' + qrSize + 'x' + qrSize +
                    '&chl=' + encodeURIComponent(url) + '&choe=UTF-8';
    qrImg.src = qrSrc;
    qrImg.alt = 'QR Code - ' + (USER.nm || 'مشترك');
...
window.onload = function () {
  ...
  // راديو: إظهار/إخفاء قائمة الفئة المحددة
  document.querySelectorAll('input[name="fixedCatOpt"]').forEach(radio => {
    radio.addEventListener('change', function() {
      const wrap = document.getElementById('fixedCatSelectWrap');
      wrap.style.display = this.value === 'yes' ? 'flex' : 'none';
    });
  });
